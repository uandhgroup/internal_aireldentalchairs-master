frappe.ready(function() {
	...
	if(plan_name) {
		frappe.call({
			method: 'internal_aireldentalchairs.www.pricing.index.get_plan_details',
			args: { plan_name },
			callback: function(r) {
				if (r.exc) return;
	...
	frappe.call({
		method:"internal_aireldentalchairs.api.load_dropdowns",
		callback: function(r) {
...
setup_signup = function(page) {
	...
	page.find('.btn-request').off('click').on('click', function() {
		...
		frappe.call({
			method: 'internal_aireldentalchairs.api.signup',
			args: args,
			type: 'POST',
			btn: $btn,
			callback: function(r) {
	...
	function check_if_available(subdomain, callback) {
		setTimeout(function() {
			frappe.call({
				method: 'internal_aireldentalchairs.api.check_subdomain_availability',
